...
namespace Peregrine
{
  ...
  count(DataGraphT &&data_graph, const std::vector<SmallGraph> &patterns, uint32_t nworkers, int my_rank, int world_size)
  {
    // initialize
    ...
    if (world_size == 1) {
      num_tasks = all_tasks_num;
      startPt = 0;
      endPt = num_tasks;
    } else {
      num_tasks = all_tasks_num / (world_size);
      startPt = my_rank * num_tasks;
